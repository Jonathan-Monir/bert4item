...
def task_test(path):
    df_rows = pd.read_parquet(path)
    df_rows = df_rows.rename(columns={"CustomerId":"uid", "MenuItemId":"iid", "OrderDate":"timestamp"})

def task_prepare_items(args):
    print("task: prepare items")
    data_dir = os.path.join(args.data_root, args.name)
    os.makedirs(data_dir, exist_ok=True)
    print(os.path.join(args.rough_root, args.name, 'items.parquet'))
    
    # load your Parquet
    df_rows = pd.read_parquet(os.path.join(args.rough_root, args.name, 'items.parquet'))
    
    # Convert OrderDate to datetime first
    df_rows['OrderDate'] = pd.to_datetime(df_rows['OrderDate'])
    
    # Convert to Unix timestamp (integer seconds) - this is crucial!
    df_rows['timestamp'] = df_rows['OrderDate'].astype('int64') // 10**9
